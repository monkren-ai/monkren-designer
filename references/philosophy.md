...
> 回答「我信什么？」——设计审查与创作背后的信念体系。

---

## 1. 核心哲学深度论述

### 1.1 从 existing context 出发审查

好的设计**一定**是从已有上下文长出来的。审查时第一件事：检查设计是否基于真实的 design system / UI kit / codebase / Figma / 品牌规范。**凭空做 hi-fi 一定会产出 generic 的作品**——这是审查中最常见的根本性问题。

审查检查项：
- 设计是否引用了项目已有的 design system / 组件库？
- 如果涉及具体品牌，是否使用了真实的品牌资产（logo/产品图/UI截图/色值）？
- 是否存在「没有上下文就硬做」的痕迹？（通用渐变、无品牌识别度的配色）

### 1.2 反 AI slop

**AI slop = AI 训练语料里最常见的"视觉最大公约数"**。审查时必须识别并标记。

**信念**：AI slop 的判断边界是「品牌本身用」——品牌 spec 里明写了用紫渐变，那就用，此时它不再是 slop，是品牌签名。不是品牌签名却出现在设计里的 AI 训练语料残留，就是 slop。

**8 类典型 AI slop 元素**（激进紫色渐变 / Emoji 作图标 / 圆角卡片+左彩色 border accent / SVG imagery / CSS 剪影 / 烂大街字体 / 赛博霓虹 / 过度动效与微交互）及详细判断规则 → `references/aesthetics.md`（反 AI slop 完整黑名单 + 替换建议）

### 1.3 系统优先，不要填充

**Don't add filler content**。审查时检查：每个元素是否 earn its place？空白是否被构图解决而非靠编造内容填满？**One thousand no's for every yes**。尤其警惕：
...
审查时检查：没图标的地方是否留了灰色方块+文字标签，而不是画烂 SVG？没数据的地方是否写了 `<!-- 等用户提供真实数据 -->`，而不是编造假数据？**Hi-fi 里，一个诚实的 placeholder 比一个拙劣的真实尝试好 10 倍**。

### 1.5 品牌的哲学：被认出来

品牌的本质是「它被认出来」。认出来靠什么？按识别度排序：Logo > 产品图 > UI 截图 > 色值 > 字体。

这就是为什么审查时必须检查品牌资产是否被正确使用——不是装饰问题，是识别度问题。

→ 品牌资产合规检查项、违规模式、5-10-2-8 素材质量门槛 → 详见 `references/design-system.md`（核心资产协议）
...
**品位基线**（字体/色彩/信息密度/细节签名的首选与避免）→ `references/aesthetics.md`（品位锚点详细解释 + 具体首选/避免对照）

---

## 4. 设计方向顾问完整规则

> v3.0 重写(2026-06-25):从"3 条触发 + 松散字段"升级为"5 类触发 + 6 字段固定结构 + 5 铁律质量保证 + 3 方向横评"。底层数据见 §5 项目类型适配、§6 真实产品案例、§7 约束过滤。

### 4.1 触发条件(5 类场景)

设计方向顾问**不是默认动作**,只在以下 5 类场景中触发。agent 必须先识别场景,再决定输出深度。
...
```
你的项目处于什么阶段?
A. 冷启动 (0→1) — 没有 design system,从零开始
B. 增量设计 — 已有 system,新增模块/页面
C. 方向诊断 — 已有设计,想识别流派
D. 瓶颈突破 — 陷同质化,想换风格
E. 标杆对齐 — 想对标某个产品/品牌
```

**简化模式**:如果用户明确说"我就要这种风格"或"我只要 1 个方向"→ 跳过 3 方向推荐,直接给 1 个深度方向(仍应用 6 字段结构)。
...
- 用户只要评分提升建议 → 走 score-improvement 模板
...
- 因为 [你的项目是 X 类型,这个哲学在 Y 方面有 Z 优势](引用 §6 真实产品案例)
...
- ❌ [常见反模式 2] → ✅ [修复方向]
- ❌ [常见反模式 3] → ✅ [修复方向]
[反模式数据来源 → §2 该流派的"反模式检测"小节]
```

### 4.4 3 方向横评对比表

3 个方向输出后,必须给一张对比表,让用户一眼选。
...
```
1. [5 分钟动作] — [如:打开 [产品案例 URL] 截图研究]
2. [30 分钟动作] — [如:提取色板到 Figma Tokens / 写 1 页示例]
3. [1 天动作] — [如:用选定风格做 1 个完整原型页验证]
```

### 4.6 输出质量铁律(5 铁律,违反任一不交付)
...
**降级处理**:MCP 不可用时,方向建议仅包含文字描述,不附带视觉参考。方向建议的有效性不依赖视觉参考。

→ 详见 `references/lazyweb-integration.md`(参考搜索完整规则)

### 4.8 ❌ 禁止从同一流派推荐 2 个以上

差异化不够,用户看不出区别。3 个方向必须横跨至少 3 个不同流派(同铁律 4)。

### 40 种设计哲学速查表(10 流派 × 4 方向)

| 流派 | 方向 | 视觉气质 | 适合作为 |
|------|------|---------|---------|
| **信息建筑派** | 01 Pentagram 信息建筑 | 理性、克制、网格 | 安全/专业选择 |
| | 02 Stamen Design 数据景观 | 地图学、有机、温暖 | 数据/景观选择 |
| | 03 Information Architects 信息架构 | 内容优先、系统字体、性能 | 内容/阅读选择 |
| | 04 Fathom 科学可视化 | 科学、精确、严谨 | 科学/数据选择 |
| **运动诗学派** | 05 Locomotive 沉浸叙事 | 视差、电影化、沉浸 | 沉浸/叙事选择 |
| | 06 Active Theory 数字雕塑 | 3D粒子、霓虹、WebGL | 数字/3D选择 |
| | 07 Field.io 运动诗学 | 算法、生成、抽象 | 算法/生成选择 |
| | 08 Resn 交互探索 | 游戏化、插画、非线性 | 交互/探索选择 |
| **极简主义派** | 09 Experimental Jetset 极简概念 | 概念、蒙德里安、反商业 | 文化/概念选择 |
| | 10 Müller-Brockmann 瑞士网格 | 数学网格、绝对对齐、功能 | 系统/网格选择 |
| | 11 Build 奢侈极简 | 奢侈留白、微妙字重 | 奢侈/品牌选择 |
| | 12 Sagmeister & Walsh 实验先锋 | 快乐、手工、意外色彩 | 快乐/实验选择 |
| **实验先锋派** | 13 Zach Lieberman 创意编码 | 手绘算法、黑白、诗意 | 创意/编码选择 |
| | 14 Raven Kwok 生成艺术 | 分形、递归、参数化 | 参数/系统选择 |
| | 15 Ash Thorp 温暖赛博 | 电影光影、温暖赛博 | 电影/概念选择 |
| | 16 Territory Studio 未来界面 | FUI、全息、数据流 | 未来/界面选择 |
| **东方哲学派** | 17 Takram 设计工程 | 概念原型、柔和科技 | 思辨/工程选择 |
| | 18 Kenya Hara 白 | 极致留白、白的层次 | 留白/感知选择 |
| | 19 Irma Boom 非线性 | 非线性、边界游戏 | 书籍/非线性选择 |
| | 20 Naoto Fukasawa 无意识设计 | 极致克制、环境融合 | 日式/克制选择 |
| **野蛮生长派** | 21 Pascal Devoyre 裸露真实 | 原始、诚实、反叛 | 反叛/真实选择 |
...
| **复古未来派** | 33 Syd Mead 视觉未来主义 | 经典、流线、太空 | 经典/太空选择 |
...
| **极繁主义派** | 37 David Carson 解构排版 | 叛逆、直觉、解构 | 叛逆/文化选择 |
| | 38 Paula Scher 信息地图 | 地图、密度、文化 | 地图/品牌选择 |
| | 39 Peter Saville 音乐封面极繁 | 封面、拼贴、文化 | 音乐/文化选择 |
| | 40 Kelly Wearstler 空间极繁 | 奢华、历史、感官 | 奢华/空间选择 |

详细风格库 + AI 提示词模板 → 见本文「2. 40 种设计哲学完整风格库」。
项目类型适配 → §5
真实产品案例 → §6
约束过滤维度 → §7
...
- 涉及到具体品牌时,需执行品牌资产提取协议(详见 implementation.md)
...
当用户请求Demo（触发词：demo/演示/看看效果/出个html/做个示例/generate demo）或设计方向顾问输出后用户选定方向并要求可视化时，按以下规范生成单文件HTML Demo。

### 12.1 触发条件

满足以下任一条件时生成Demo：
1. 用户消息包含触发词：demo、演示、看看效果、出个html、做个示例、generate demo、show me
2. 设计方向建议输出后，用户回复"方向1/demo方向2/第一个/生成demo"等
...
**强制要求**：
- **单文件HTML**：所有CSS内联在`<style>`标签中，不依赖任何外部CSS/JS/字体文件
- **系统字体兜底**：使用系统字体栈（按§2字体栈顺序，第一个放系统可用字体），不引入外部字体（确保离线可用）
- **图片占位**：使用CSS渐变/SVG pattern/纯色块/ASCII字符占位，**不使用外部图片URL**（避免失效）
- **响应式**：至少适配3个断点——桌面(1440px)、平板(768px)、移动端(375px)
- **内联CSS变量**：色板、字体、间距用CSS变量声明在:root中（符合Token思路）

**页面内容**（默认品牌落地页，用户指定页面类型时按§11模式库）：
- 顶部demo-note条：标注流派+哲学名+该风格核心签名关键词
- Navigation（体现该流派导航特征）
- Hero区域（体现该哲学**最强视觉签名**，这是判断Demo质量的核心）
- 3个Feature卡片
- 1个CTA区域
- Footer

**文件位置与命名**：
- 保存到：`.monkren/demos/{philosophy-slug}-{page-type}-{YYYY-MM-DD}.html`
- kebab-case命名，如：`mazzini-dashboard-2026-06-26.html`、`locomotive-landing-2026-06-26.html`
- 如果`.monkren/demos/`不存在则创建

### 12.3 质量自检清单（生成后必须逐项检查）

- [ ] **哲学特征体现**：对照§2核心特征，至少3个该哲学的签名元素在Demo中可见（如野蛮UI必须有粗边框+硬阴影+零圆角）
- [ ] **色板准确**：使用§2中该哲学的OKLch色板（转为CSS oklch()值），不随意换色
- [ ] **无AI slop**：没有无意义的装饰元素、没有lorem ipsum（中文用中文，英文用合理英文）、没有紫色渐变
- [ ] **可交互**：所有按钮/链接/卡片有hover态样式
- [ ] **对比度**：正文文字与背景对比度≥4.5:1（WCAG AA）
- [ ] **响应式**：375px宽度下内容不溢出、不横向滚动
