...
> 对应 skill: `skills/01-research/discovery-questions/`
...
> 对应 skill: `skills/01-research/frontend-aesthetic-direction/`
...
> 对应 skill: `skills/02-create/wireframe/`
...
然后建议 `skills/03-execute/make-a-prototype/`(hi-fi 交互)、`skills/_deprecated/make-a-deck/`(wireframe 是为演示做的)、或再一轮低保真。**不要在 wireframe 本身上投入 hi-fi 打磨**——它们已完成使命。
...
> 对应 skill: `skills/02-create/generate-variations/`
...
静态变体 side-by-side 用 design canvas;变体共享结构、在少数轴上不同时用 **tweak**(`skills/_deprecated/make-tweakable/`)。流程变体每个作为 canvas 内一个小故事板。**不要产出 v1.html / v2.html / v3.html**——一个文件,所有变体可见或可 toggle。
...
然后建议下一步:选中方向的精修轮、不同轴的另一轮变体、`skills/03-execute/make-a-prototype/` 走交互、或 `skills/04-review/polish-pass/` 准备 ship。

---

## 7. 原型制作方法(Make a Prototype)

> 对应 skill: `skills/03-execute/make-a-prototype/`
...
构建前确认:**流程**(屏幕、入口、目标状态——列成 list);**保真度**(hi-fi 或 mid-fi);**设备框**(desktop browser、iOS、Android、macOS window);**变体**(一个流程或几个对比);**品牌/design system**(永远确认——没有就先调 `skills/01-research/frontend-aesthetic-direction/`);**样例数据**(真实感内容,不要 Lorem ipsum)。
...
> 对应 skill: `skills/_deprecated/make-a-deck/`
...
构建前确认:**受众**(决定语调和密度);**格式**(长宽比——默认 16:9——和幻灯片数:10 分钟 deck ~10 张,30 分钟 ~20-25);**语调**(formal corporate、casual internal、marketing-bold、technical);**源内容**(画之前读任何 PRD 或已有材料);**speaker notes**(默认关,明确请求才开);**品牌/design system**(永远确认——没有就先调 `skills/01-research/frontend-aesthetic-direction/`)。
...
在 preview 里从头到尾走 deck:多 viewport 尺寸下缩放、计数器正确递增、键盘导航(箭头、space)工作、无溢出幻灯片边界、字号满足 24px+ 最小、对比度过 WCAG(调 `skills/04-review/accessibility-audit/` 做彻底检查)。
...
> 对应 skill: `skills/_deprecated/make-tweakable/`
...
> 对应 skill: `skills/_deprecated/design-system-extract/`
...
> 对应 skill: `skills/_deprecated/component-extract/`
...
然后建议下一步:`skills/_deprecated/design-system-extract/` 提取 token(如还没做)、把库建成真实代码、对组件跑 `skills/04-review/polish-pass/`、或对库加 `skills/_deprecated/make-tweakable/`。**总结**:按类别清单化的组件、标记的不一致和 gap、输出文件路径、推荐下一步。
